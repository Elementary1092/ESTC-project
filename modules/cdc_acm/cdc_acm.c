...
{
	app_usbd_class_inst_t *cdc_acm_class = app_usbd_cdc_acm_class_inst_get(acm);
...
ssize_t cdc_acm_echo(app_usbd_cdc_acm_t const *acm, char *buf, size_t buf_size)
{
...
ssize_t cdc_acm_read(app_usbd_cdc_acm_t const *acm, char *buf, size_t buf_size)
{
	char temp_buf[CDC_ACM_TEMP_BUF_SIZE] = {0};
	ret_code_t ret;
...
void cdc_acm_write(app_usbd_cdc_acm_t const *acm, const char *str, size_t str_len)
{
	(void)app_usbd_cdc_acm_write(acm, buf, buf_size);
}
